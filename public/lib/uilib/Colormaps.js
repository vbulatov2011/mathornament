...
function filterZeroLengthIntervals(cmdata) {
    if (!cmdata || cmdata.length < 3) return cmdata;
    const count = Math.floor(cmdata.length / 3);
    const filtered = [];
    const eps = 1e-5;

    let i = 0;
    while (i < count) {
        const posX = cmdata[i * 3][0];
        const leftColor = cmdata[i * 3 + 1];
        let j = i;

        // Group points at the exact same position x (within eps)
        while (j + 1 < count && Math.abs(cmdata[(j + 1) * 3][0] - posX) < eps) {
            j++;
        }

        const rightColor = cmdata[j * 3 + 2];
        filtered.push([posX, 0, 0, 0]);
        filtered.push([...leftColor]);
        filtered.push([...rightColor]);
...
export function updateColormapTexture(gl, colormap){
  if(!gl || !colormap || !colormap.data) return null;
  let rawCmData = colormap.data;
  let cmdata = filterZeroLengthIntervals(rawCmData);
  let count = cmdata.length;
  var data = new Float32Array(4*count);
  vec4arrayToArray(cmdata, data);

  let tex = rawCmData.tex || colormap.tex;
  if(!tex){
    tex = createDataTexture(gl);
  }
...
export function getColormapTexture(gl, options){
  ...
  let tex = cmdata.tex || colormap.tex;
  
  if(isDefined(tex) && !colormap.dirty){
    // already allocated and clean
    return tex;
  }
  
  return updateColormapTexture(gl, colormap);
}
...
/**
 * Convert legacy colormap points data array into an array of intervals:
 * [ { x0, x1, color0: [r,g,b,a], color1: [r,g,b,a] }, ... ]
 */
export function convertPointsToIntervals(data) {
    const intervals = [];
    if (!data || data.length < 6) {
        return [{ x0: 0.0, x1: 1.0, color0: [0, 0, 0, 1], color1: [1, 1, 1, 1] }];
    }
    const count = Math.floor(data.length / 3);
    for (let i = 0; i < count - 1; i++) {
        const x0 = data[i * 3][0];
        const color0 = data[i * 3 + 2]; // right color of point i
        const x1 = data[(i + 1) * 3][0];
        const color1 = data[(i + 1) * 3 + 1]; // left color of point i+1

        if (x1 > x0 + 1e-5) {
            // Check if both colors are fully transparent gap
            const isTransparentGap = (color0[3] === 0 && color1[3] === 0);
            if (!isTransparentGap) {
                intervals.push({
                    x0: parseFloat(x0.toFixed(4)),
                    x1: parseFloat(x1.toFixed(4)),
                    color0: [color0[0], color0[1], color0[2], color0[3] ?? 1.0],
                    color1: [color1[0], color1[1], color1[2], color1[3] ?? 1.0]
                });
            }
        }
    }
    if (intervals.length === 0) {
        intervals.push({ x0: 0.0, x1: 1.0, color0: [0, 0, 0, 1], color1: [1, 1, 1, 1] });
    }
    return intervals;
}

/**
 * Convert intervals array into GPU point sequence format (data array)
 * Automatically fills gaps before first, between, and after last interval with transparent pixels [0,0,0,0].
 */
export function convertIntervalsToPoints(intervals) {
    if (!intervals || intervals.length === 0) {
        intervals = [{ x0: 0.0, x1: 1.0, color0: [0, 0, 0, 1], color1: [1, 1, 1, 1] }];
    }

    // Sort intervals by x0, ensuring x0 < x1
    const validIntervals = intervals
        .map(iv => ({
            x0: Math.max(0, Math.min(1, Math.min(iv.x0, iv.x1))),
            x1: Math.max(0, Math.min(1, Math.max(iv.x0, iv.x1))),
            color0: [...iv.color0],
            color1: [...iv.color1]
        }))
        .filter(iv => iv.x1 > iv.x0 + 1e-5)
        .sort((a, b) => a.x0 - b.x0);

    if (validIntervals.length === 0) {
        validIntervals.push({ x0: 0.0, x1: 1.0, color0: [0, 0, 0, 1], color1: [1, 1, 1, 1] });
    }

    const transparent = [0, 0, 0, 0];
    const pointsMap = new Map();

    function setPointLeft(x, color) {
        const key = x.toFixed(6);
        if (!pointsMap.has(key)) {
            pointsMap.set(key, { x, left: [...color], right: [...color] });
        }
        pointsMap.get(key).left = [...color];
    }

    function setPointRight(x, color) {
        const key = x.toFixed(6);
        if (!pointsMap.has(key)) {
            pointsMap.set(key, { x, left: [...color], right: [...color] });
        }
        pointsMap.get(key).right = [...color];
    }
    ...
    for (let k = 0; k < validIntervals.length; k++) {
        ...
        if (iv.x0 > curX + 1e-5) {
            // Gap between curX and iv.x0
            if (curX > 0.0) {
                setPointRight(curX, transparent);
            } else {
                setPointLeft(0.0, transparent);
                setPointRight(0.0, transparent);
            }
            setPointLeft(iv.x0, transparent);
            setPointRight(iv.x0, iv.color0);
        } else {
            // Abutting or starting at curX
            if (iv.x0 === 0.0 && curX === 0.0) {
                setPointLeft(0.0, iv.color0);
                setPointRight(0.0, iv.color0);
            } else {
                setPointRight(iv.x0, iv.color0);
            }
        }
        ...
        // End of interval iv
        setPointLeft(iv.x1, iv.color1);
        setPointRight(iv.x1, iv.color1);
        curX = iv.x1;
    }

    if (curX < 1.0 - 1e-5) {
        // Gap between last interval end and 1.0
        setPointRight(curX, transparent);
        setPointLeft(1.0, transparent);
        setPointRight(1.0, transparent);
    }

    const sortedPoints = Array.from(pointsMap.values()).sort((a, b) => a.x - b.x);
...
let getColormap = (name)=> {
    const cm = colormaps[name];
    if (cm && !cm.intervals) {
        cm.intervals = convertPointsToIntervals(cm.data);
    }
    return cm;
}
...
// Initialize intervals on all default colormaps
Object.values(colormaps).forEach(cm => {
    if (!cm.intervals) {
        cm.intervals = convertPointsToIntervals(cm.data);
    }
});
...
export const Colormaps =  {
    ...
    convertPointsToIntervals: convertPointsToIntervals,
    convertIntervalsToPoints: convertIntervalsToPoints
}

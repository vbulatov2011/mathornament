...
vec4 texture_box_1(sampler2D data, vec2 uv){

    vec2 res = vec2(textureSize(data, 0));
    
    vec2 q = floor(uv * res + 0.5)/res; 
    
    return texture(data, q);
    
}

vec4 texture_box(sampler2D data, vec2 uv){

    ivec2 iq = ivec2(uv*vec2(textureSize(data, 0)));    
    
    return texelFetch(data, iq, 0);

...
//
//  smooth-linear step: zero derivative at edge0 (smooth start, like smoothstep),
//  transitioning to linear-like slope at edge1.
//  Constructed by evaluating smoothstep over a doubled interval [edge0, edge0+2*(edge1-edge0)],
//  so that edge1 falls at its inflection point (maximum slope), then scaling by 0.5.
//
float smoothLinearStep(float edge0, float edge1, float x) {
  return clamp(0.5 * smoothstep(edge0, edge0 + 2.0 * (edge1 - edge0), x), 0.0, 1.0);
}

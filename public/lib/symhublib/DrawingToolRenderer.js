...
const myPrograms = {
    drawDot:       progDrawDot,
    drawMultiDot:  progDrawMultiDot,
    drawSegment:   progDrawSegment,
};
...
function DrawingToolRenderer(options){
    ...
    function init(options){
        ...
        myProgs.getProgram(mGL, 'drawDot'); // triggers compileAll
    ...
    function drawDot(pnt){
        ...
        let buffer = mSimulation.getPatternData().getMainBuffer();
        ...
        let bc = mConfig.brush;
        let group = mSimulation.getGroup ? mSimulation.getGroup(): null;
        if(bc.symmetry)

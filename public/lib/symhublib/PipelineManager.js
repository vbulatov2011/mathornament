...
function PipelineManager(options = {}) {
    ...
    // MATHORNAMENT PATCH — the drawing tools need these two.
    // DrawingToolRenderer calls simulation.getGroup() (unguarded in drawSegment
    // and drawPoint) and simulation.applySymmetry() after each stroke. The
    // PipelineManager refactor never exposed them, so any drag with the pencil
    // threw "mSimulation.getGroup is not a function" and nothing was painted.
    function getGroup() {
        return mGroup;
    }

    /** Re-run only the symmetrization workers, to fold a fresh stroke into the
     *  fundamental domain without advancing the simulation. */
    ...
    const myself = {
        ...
        addEventListener,
        setGroup,
        getGroup,        // MATHORNAMENT PATCH: required by the drawing tools
        applySymmetry,   // MATHORNAMENT PATCH: required by the drawing tools
        initSimulation,
        doStep,
        getPatternData,

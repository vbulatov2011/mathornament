...
function SymRenderer(options) {
    ...
    function onResize() {
        ...
        // MATHORNAMENT PATCH — resize the backing store BEFORE recomputing the
        // transform. onCanvasResize() caches width/height/aspect from the
        // canvas, but the backing store was only resized later, inside
        // renderFrame() -> resizeCanvases(), which never re-notifies the
        // transform. The cached aspect therefore stayed at the PRE-resize
        // shape: rotating a phone left every tile stretched until reload.
        // onCanvasSize() (startup) already uses this order.
        resizeCanvases();
        mCanvasTransform.onCanvasResize();
        scheduleRepaint();
    }
    ...
    function setTool(toolName) {
        ...
        case TOOL_PICK:
            ...
            if (!mDrawingToolHandler) {
                ...
                // MATHORNAMENT PATCH: hand the handler a live view of the active
                // tool, otherwise it cannot tell picking from drawing.
        ...
        case TOOL_DRAW:
            ...
            mDrawingToolHandler = DrawingToolHandler({
                renderer: mDrawingToolRenderer,
                getToolName: () => mConfig.tools.toolName,   // MATHORNAMENT PATCH
            });
            mActiveTools = [mDrawingToolHandler, mNavigator];
            break;
        case TOOL_PATTERN_TRANSFORM:

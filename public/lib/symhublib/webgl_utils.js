...
//
// MATHORNAMENT PATCH — texture filtering has to match what the GPU can do.
//
// A 32-bit float texture may only use LINEAR filtering when the device exposes
// OES_texture_float_linear. Most Android GPUs do not (desktop and iOS do), and
// a float texture set to LINEAR is *incomplete*: every sample reads zero, so
// the simulation renders as a blank canvas with no error anywhere. Degrade to
// NEAREST in that case — the simulation itself is unaffected, only the
// interpolation when the buffer is magnified.

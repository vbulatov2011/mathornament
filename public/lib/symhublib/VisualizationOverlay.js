...
export function VisualizationOverlay(par={}){
    ...
    if (par.config) Object.assign(mConfig, par.config);  // apply caller-supplied overrides
    const mIdRef = { id: par.id ?? '' };  // editable layer name/id
    let mOnIdChange = null;
    const overlayParams = makeOverlayParams(mConfig, onChange);
    // Prepend the editable 'id' field so it appears first in the UI folder.
    let mParams = { id: ParamString({ obj: mIdRef, key: 'id', name: 'id', onChange: () => { if (mOnIdChange) mOnIdChange(); } }), ...overlayParams };
    let mOnChange = null;
    let mPrograms = null;
    let mGLCtx = null;

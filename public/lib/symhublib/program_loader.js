...
function initFragment(frag){
    ...
    if(!isDefined(frag.txt))
        throw new Error(`fragments collection ${frag.obj.getName()} has no such fragment: ${frag.id}`);
...
//
//  Generic lazy/eager GPU program registry factory.
//
//  programs   — dict of { name: programDef } — same shape as used by buildProgramsCached.
//  compileAll — if true, compile every program on the first getProgram() call;
//               if false (default), compile each program only when first requested.
//
//  Returns { getProgram(gl, name) }.
//
function programBuilder(programs, compileAll = false) {
    const DEBUG = true;
    const MYNAME = 'programBuilder';
    let mGL = null;

    function ensureCompiled(gl, pr, progName) {
        //if(false)console.log(`${MYNAME}.ensureCompiled(${progName})`);
        if (pr.program || pr.compileFailed) return;
        if(DEBUG)console.log(`${MYNAME}.ensureCompiled(${progName}) compiling...`);
        buildProgramsCached(gl, { [progName]: pr });
        if (!pr.program) {
            console.error(`${MYNAME} failed to compile '${progName}'`);
            pr.compileFailed = true;
        }
    }

    function getProgram(gl, progName) {

        //if(DEBUG)console.log(`${MYNAME}getProgram(${progName})`);

        const pr = programs[progName];
        if (!pr) throw new Error(`programBuilder: program '${progName}' not found`);

        if (compileAll) {
            if (!mGL) {
                mGL = gl;
                for (const [name, prog] of Object.entries(programs))
                    ensureCompiled(gl, prog, name);
            }
        } else {
            ensureCompiled(gl, pr, progName);
        }
